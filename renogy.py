...
class Device(minimalmodbus.Instrument):
    ...
    def product_model(self):
        return self.read_string(self.PRODUCT_MODEL, 8).strip()
    ...
    def battery_current(self):
        return self.read_register(self.CHARGE_MAX_CURRENT, 2, signed=True)
    ...
    def power_daily(self):
        return self.read_register(self.POWER_DAILY)

    def power_total(self):
        return self.read_long(self.POWER_TOTAL)
    ...
    def alarm_a(self):
        status = [
            ...
            "reserved#3",
            "reserved#4",
            "controller_inside_over_temp",
            "alternator_input_over_current",
            "reserved#7",
            "reserved#8",
            "alternator_input_over_voltage_protection",
            "starter_battery_reverse_polarity",
            "bms_over_charge_protection",
            "low_temperature_cutoff",
        ]
        return self.name_status_bits(self.ALARM_A, status)

    def alarm_b(self):
        status = [
            "battery_over_discharged",
            "battery_over_charged",
            "battery_under_voltage_warning",
            "reserved#4",
            "reserved#5",
            "controller_inside_temp_too_high",
            "battery_over_temp",
            "solar_input_too_high",
            "reserved#9",
    ...
    def all(self):
        y = filter(
            lambda x: x.startswith("battery")
            or x.startswith("temperature")
            or x.startswith("voltage")
            or x.startswith("current")
            or x.startswith("power")
            or x.startswith("charge")
            or x.startswith("product")
            or x.startswith("alarm"),
            dir(self),
        )
        methods = list(y)
        d = {}
